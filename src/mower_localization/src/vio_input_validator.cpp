...
{
  if (state_ == VioState::kUnconfigured || state_ == VioState::kError) {++rejected_images_; return false;}
  ...
  {++rejected_images_; set_state(VioState::kDegraded, "Image and calibrated CameraInfo do not match"); return false;}
  if (has_last_image_stamp_ && image.stamp_ns <= last_image_stamp_ns_) {
    ++rejected_images_; set_state(VioState::kDegraded, "image timestamp is duplicate or out of order"); return false;
  }
  if (imu_buffer_.take_imu_through(image.stamp_ns).empty()) {
    ++rejected_images_; set_state(VioState::kInitializing, "waiting for IMU samples through image timestamp"); return false;
  }
...
{if (state_ != VioState::kUnconfigured && state_ != VioState::kError) {set_state(VioState::kLost, "sensor input timeout");}}
...
{
  switch (state) {
    case VioState::kUnconfigured: return "UNCONFIGURED"; case VioState::kInitializing: return "INITIALIZING";
