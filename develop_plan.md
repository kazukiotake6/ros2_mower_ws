...
### 13.8 OV9281内部キャリブレーション実施状況（2026-08-15）

#### 完了済み

| 項目 | 結果・成果物 |
| --- | --- |
| OV9281の検出・映像取得 | Pi 5のPiSP対応libcameraでOV9281を認識し、1280x800でフレーム取得を確認した。 |
| ROS 2への配信 | `camera_ros`で`/camera/image_raw`と`/camera/camera_info`を配信し、較正値の反映を実機で確認した。 |
| 単眼内部パラメータ取得 | 内側コーナー8x6、正方形一辺0.030 mのチェッカーボードで最終80枚を取得し、K/D/R/Pを算出した。 |
| 成果物の版管理・既定適用 | `mower_camera/config/ov9281_1280x800.yaml`を追加し、`libcamera.yaml`の既定`camera_info_url`から参照する。 |
| ソフトウェア検証 | `mower_camera`のビルド、単体テスト、YAMLをロードした実機`CameraInfo`の1280x800・K/D/P一致を確認した。 |

#### 未完了・受入れ保留

| 項目 | 理由・次の完了条件 |
| --- | --- |
| 内部較正のVIO受入れ | 初回56枚は独立RMS 2.573253 pxで却下した。最終80枚は保存・適用済みだが、このPiのOpenCVで独立RMSを取得できていない。固定露光・固定ゲインで再取得し、合意済み閾値以下のRMS、直線性、VIO追跡を記録するまでVIOへ投入しない。 |
| 露光・ゲイン・配信品質 | 自動露光・自動ゲインで内部較正を実施した。固定値での再較正、実効FPS、フレーム欠落、キュー遅延、CPU・温度をrosbag2と`/diagnostics`で評価する。 |
| Camera--IMU時刻同期 | フレーム開始、BMI270 IRQ、ROS時刻のオフセット・最大ジッタ・ドリフトを未計測である。BMI270 FIFO/IRQ/SENSORTIME対応後にロジアナで計測する。 |
| Camera--IMU外部パラメータ | カメラとIMUの取付変換は未較正である。時刻同期の受入れ後に手押しデータで外部較正し、YAMLと手順を版管理する。 |
| IMX296プロファイル | IMX296のPi 5での検出、実効format/FPS、内部較正は未実施である。OV9281の値を流用しない。 |
| 実走耐性評価 | 防水窓、日照、芝・泥、振動、低FPS、カメラ断を含む試験と上位停止要求の統合は未実施である。 |

#### 次の実施順序

1. OV9281を最終取付状態・固定露光・固定ゲインで再撮影し、再投影RMSを合意済み閾値と比較して内部較正を受入れる。
2. rosbag2で画像、CameraInfo、IMU、diagnosticsを同時記録し、フレーム欠落・遅延・負荷を測定する。
3. BMI270 FIFO/IRQ/SENSORTIME実装後に、Camera--IMU時刻同期と外部パラメータ較正を行う。
4. VIO手押し評価、IMX296比較、芝生環境の耐性評価へ進む。

この節の完了済みは内部較正成果物の取得・適用までを意味し、VIOの精度受入れ、安全機能の受入れ、またはCamera--IMU較正の完了を意味しない。
