...
OV9281のPi 5上での検出、起動、1280x800・YUYV・60 Hz配信と内部パラメータ較正は13.6および13.9で完了している。ただし、現行の較正記録は自動露光・自動ゲインで取得され、最終セッションの再投影RMSも未記録である。VIOへ投入する前に、固定露光・固定ゲインで再取得したセッションについて、合意済み閾値に対するRMSと直線性を記録して受入れ確認を行う。次回以降は以下をこの順で実施する。
...
2026-09-04時点で、決定論的な合成bag生成、seedと期待状態を含むmanifest、および実時間再生による正常、CameraInfo不一致、IMU欠損・重複、画像時刻逆行、カメラ停止の自動判定を実装した。低FPS、IMU逆行、入力順序乱れ、倍率別再生、ノード再起動と復旧判定は継続項目とする。
